...
fn snaps() -> Vec<u8> {
    ...
    return vec![0x00, 0x5f, 0x87, 0xaf, 0xd7, 0xff]
        .windows(2)
...
pub fn calc_size(img: &image::DynamicImage, width: Option<u32>) -> Result<(u32, u32), &str> {
    ...
    return match (width, terminal_size()) {
        (Some(w), _) => Ok(w),
...
pub fn true_color(img: &image::DynamicImage, x: u32, y: u32) -> String {
    ...
    return format!("\x1b[48;2;{};{};{};38;2;{};{};{}m▄", r1, b1, g1, r2, b2, g2);
}
...
pub fn indexed(img: &image::DynamicImage, x: u32, y: u32) -> String {
    ...
    return format!("\x1b[38;5;{};48;5;{}m▄", fg, bg);
}
